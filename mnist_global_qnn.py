...
class MNIST_QNN_Classifier(nn.Module):
    def __init__(self, num_qnn_layers: int, num_qnn_blocks: int, m: int = 2, num_qnn_qubits: int = 6,
                 if_local=False):
        """
        初始化QNN分类器模型。
        """
        ...
        self.total_qubits = m + num_qnn_qubits  # 3 + 6 = 9
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(x.shape[0]):
            ...
            # --- 6. 测量并计算输出 ---
            # 二分类，只在第4个比特（索引3，即QNN的第一个比特）上测量
            measurement_index = self.m  # 索引为3
...
def main(num_qnn_blocks: int = 2, num_qnn_layers: int = 2, epochs: int = 20, learning_rate: float = 0.01,
    ...
    # --- 1. 数据准备 (MNIST 0 vs 1) ---
    print("1. 准备MNIST数据集 (类别 0 和 1，或其他)...")
    ...
    test_indices = get_class_indices(full_test_dataset, [9, 6])
    ...
    model = MNIST_QNN_Classifier(num_qnn_layers=num_qnn_layers, num_qnn_blocks=num_qnn_blocks, if_local=if_local).to(
        DEVICE)
